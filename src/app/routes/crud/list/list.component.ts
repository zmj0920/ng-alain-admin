...
export class CrudListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const click$ = fromEvent($attachment, 'click')
      .pipe(
    ...
    const upload$ = fromEvent($attachment, 'change')
      .pipe(
  ...
  private postUploadChunk(i: { fileinfo: any; file: File }) {
    return ajaxPost(`${apiHost}/upload/chunk`, i.fileinfo)
      .pipe(
        map((r) => {
  ...
  private postMergeStatus(r: ChunkMeta) {
    return ajaxPost(`${apiHost}/upload/chunk/${r.fileKey}`)
      .pipe(
        mapTo({
